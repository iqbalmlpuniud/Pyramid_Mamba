...
class PatchExpand2D (nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.expand (x)
        x = rearrange (
            x,'b h w (p1 p2 c)-> b (h p1) (w p2) c',p1 = self.dim_scale,p2 = self.dim_scale,c = C // self.dim_scale)
        x = self.norm (x)
        return x


def split_image(img,num_patches):
    B,C,L = img.shape
    H = W = int (L ** 0.5)
    ...
    for i in range (num_patches):
        for j in range (num_patches):
            ...
            # Flatten each patch
            patch_flat = patch.reshape (B,C,-1)
            patches.append (patch_flat)

    return patches


def combine_patches(processed_patches,num_patches,H,W):
    B,C,patch_L = processed_patches [0].shape
    patch_H = H // num_patches
    patch_W = W // num_patches    
    combined_img = torch.zeros ((B,C,H,W),dtype = processed_patches [0].dtype,device = processed_patches [0].device)

    # Combine patches into the full image tensor
    index = 0
    for i in range (num_patches):
        for j in range (num_patches):
            # Calculate the start and end indices for this patch in the combined image
            start_H = i * patch_H
            end_H = (i + 1) * patch_H
            start_W = j * patch_W
            end_W = (j + 1) * patch_W

            # Reshape the processed patch to (B, C, patch_H, patch_W)
            patch = processed_patches [index].reshape (B,C,patch_H,patch_W)

            # Place the patch into the combined image
            combined_img [:,:,start_H:end_H,start_W:end_W] = patch

            index += 1
    L = H * W
    flattened_img = combined_img.reshape (B,C,L)
    return flattened_img


class HSCANS (nn.Module):
    def __init__(self,size=16,dim=2):
        super ().__init__ ()
        ...
        size1 = size // 2
        max_num1 = size1 ** dim
        indexes1 = np.arange (max_num1)
        locs_flat1 = indexes1

        size2 = size // 4
        max_num2 = size2 ** dim
        indexes2 = np.arange (max_num2)
        locs_flat2 = indexes2
        ...
        # print("self index",self.index_flat_inv.shape)

        locs_flat_inv1 = np.argsort (locs_flat1)
        index_flat1 = torch.LongTensor (locs_flat1.astype (np.int64)).unsqueeze (0).unsqueeze (1)
        index_flat_inv1 = torch.LongTensor (locs_flat_inv1.astype (np.int64)).unsqueeze (0).unsqueeze (1)
        self.index_flat1 = nn.Parameter (index_flat1,requires_grad = False)
        self.index_flat_inv1 = nn.Parameter (index_flat_inv1,requires_grad = False)

        locs_flat_inv2 = np.argsort (locs_flat2)
        index_flat2 = torch.LongTensor (locs_flat2.astype (np.int64)).unsqueeze (0).unsqueeze (1)
        index_flat_inv2 = torch.LongTensor (locs_flat_inv2.astype (np.int64)).unsqueeze (0).unsqueeze (1)
        self.index_flat2 = nn.Parameter (index_flat2,requires_grad = False)
        self.index_flat_inv2 = nn.Parameter (index_flat_inv2,requires_grad = False)
    ...
    def encode(self, img):
        if img.shape[-1] == self.index_flat_inv1.shape[-1]:
            #print("image shape of patch",img.shape)
            return torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(
            2, self.index_flat_inv1.expand(img.shape), img)

        elif img.shape[-1] == self.index_flat_inv.shape[-1]:
            #print ("image shape of image",img.shape)
            return torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(
            2, self.index_flat_inv.expand(img.shape), img)
        elif img.shape[-1] == self.index_flat_inv2.shape[-1]:
            #print ("image shape of image",img.shape)
            return torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(
            2, self.index_flat_inv2.expand(img.shape), img)
        else:
            raise ValueError("img shape does not match index_flat_inv or index_flat_inv1")
    ...
    def decode(self,img):
        if img.shape[-1] == self.index_flat_inv1.shape[-1]:
            return torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(
            2, self.index_flat1.expand(img.shape), img)
        elif img.shape[-1] == self.index_flat_inv.shape[-1]:
            return torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(
            2, self.index_flat.expand(img.shape), img)
        elif img.shape[-1] == self.index_flat_inv2.shape[-1]:
            return torch.zeros(img.shape, dtype=img.dtype, device=img.device).scatter_(
            2, self.index_flat2.expand(img.shape), img)
        else:
            raise ValueError("img shape does not match index_flat_inv or index_flat_inv1")
...
class SS2D (nn.Module):
    def __init__(
        ...
        dt_projs = [
            self.dt_init (self.dt_rank,self.d_inner,dt_scale,dt_init,dt_min,dt_max,dt_init_floor,**factory_kwargs) for _
            in range (self.num_direction)]
    ...
    def forward_core(self,x: torch.Tensor):
        ...
        patches = split_image (x_reshaped,2)
        b,c,l = patches [0].shape
        h = w = int (l ** 0.5)
        P0,P1,P2,P3 = [patches [i].reshape (B,C,h,w) for i in range (4)]
        ...
        def ssm_patch(x):
            ...
            if K >= 8:
                ...
                xs.append (
                    self.scans.encode (
                        torch.transpose (torch.rot90 (x,k = 1,dims = (2,3)),dim0 = 2,dim1 = 3).contiguous ().view (
                            B,-1,L)))
            ...
            if K >= 4:
                ys.append (
                    torch.transpose (
                        self.scans.decode (out_y [:,1]).view (B,-1,W,H),dim0 = 2,dim1 = 3).contiguous ().view (
                        B,-1,L))
                ys.append (
                    torch.transpose (
                        self.scans.decode (inv_y [:,1]).view (B,-1,W,H),dim0 = 2,dim1 = 3).contiguous ().view (
                        B,-1,L))
            if K >= 8:
                ys.append (
                    torch.rot90 (
                        self.scans.decode (out_y [:,2]).view (B,-1,W,H),k = 3,dims = (2,3)).contiguous ().view (
                        B,-1,L))
                ys.append (
                    torch.rot90 (
                        self.scans.decode (inv_y [:,2]).view (B,-1,W,H),k = 3,dims = (2,3)).contiguous ().view (
                        B,-1,L))
                ys.append (
                    torch.rot90 (
                        torch.transpose (self.scans.decode (out_y [:,3]).view (B,-1,W,H),dim0 = 2,dim1 = 3),k = 3,
                        dims = (2,3)).contiguous ().view (B,-1,L))
                ys.append (
                    torch.rot90 (
                        torch.transpose (self.scans.decode (inv_y [:,3]).view (B,-1,W,H),dim0 = 2,dim1 = 3),k = 3,
                        dims = (2,3)).contiguous ().view (B,-1,L))
            y = sum (ys)
        ...
        def sub_patch_ssm(patches):
            x=patches
            B,C,L=patches.shape
            patches = split_image (x,2)
            b,c,l = patches [0].shape
            h = w = int (l ** 0.5)
            P0,P1,P2,P3 = [patches [i].reshape (B,C,h,w) for i in range (4)]
            p0,p1,p2,p3 = [ssm_patch (P) for P in [P0,P1,P2,P3]]
            H = W = int (L ** 0.5)
            num_patches = 2
            processed_patches = [p0,p1,p2,p3]
            combined_img = combine_patches (processed_patches,num_patches,H,W)
            return combined_img
        ...
        num_patches = 2
        if (x.shape [-1]) > 64:
            sub_patche0,sub_patche1,sub_patche2,sub_patche3 = [sub_patch_ssm (p) for p in [p0,p1,p2,p3]]
            process_subpatches = [sub_patche0,sub_patche1,sub_patche2,sub_patche3]
            processed_patches = [p0,p1,p2,p3]
            combined_img = combine_patches (processed_patches,num_patches,H,W)
            combine_subpacthes = combine_patches (process_subpatches,num_patches,H,W)
            final_img = (combined_img + p_main + combine_subpacthes) / 3
        else:
            processed_patches = [p0,p1,p2,p3]
            combined_img = combine_patches (processed_patches,num_patches,H,W)

            final_img = (combined_img + p_main ) / 2
...
class HSSBlock (nn.Module):
    def __init__(
        ...
        self.self_attention = SS2D (
            d_model = hidden_dim,dropout = attn_drop_rate,d_state = d_state,size = size,scan_type = scan_type,
            num_direction = num_direction,**kwargs)
...
class LSSModule (nn.Module):
    def __init__(
        ...
        self.smm_blocks = nn.ModuleList (
            [
                HSSBlock (
                    hidden_dim = hidden_dim,drop_path = drop_path,norm_layer = norm_layer,
                    attn_drop_rate = attn_drop_rate,d_state = d_state,size = size,scan_type = scan_type,
                    num_direction = num_direction,**kwargs)
        ...
        self.conv55 = nn.Sequential (
            nn.Conv2d (
                in_channels = hidden_dim,out_channels = hidden_dim,kernel_size = 5,stride = 1,padding = 2,bias = False,
                groups = hidden_dim),
            nn.InstanceNorm2d (hidden_dim),
            nn.SiLU (),
        )
        self.conv77 = nn.Sequential (
            nn.Conv2d (
                in_channels = hidden_dim,out_channels = hidden_dim,kernel_size = 7,stride = 1,padding = 3,bias = False,
                groups = hidden_dim),
            nn.InstanceNorm2d (hidden_dim),
            nn.SiLU (),
        )
...
class MambaUPNet (nn.Module):
    def __init__(self,dims_decoder=[512,256,128,64],depths_decoder=[3,4,6,3],d_state=16,drop_rate=0.,attn_drop_rate=0.,
                 drop_path_rate=0.2,
...
class MFF_OCE(nn.Module):
    def __init__(self, block, layers,device, width_per_group = 64, norm_layer = None, ):
    ...
    def forward(self,x):
        ...
        if self.training:
            true_feats = sv_features
            noise_idxs = torch.randint (0,self.mix_noise,torch.Size ([true_feats.shape [0]])).to (self.device)
            noise_one_hot = torch.nn.functional.one_hot (noise_idxs,num_classes = self.mix_noise).to (
                self.device)  # (N, K)
            noise = torch.stack (
                [
                    torch.normal (0,self.noise_std * 1.1 ** (k),true_feats.shape).to (self.device)
                    for k in range (self.mix_noise)],dim = 1)
            noise = (noise * noise_one_hot.unsqueeze (-1).unsqueeze (-1).unsqueeze (-1)).sum (1)
            fake_feats = true_feats + noise

            return fake_feats.contiguous ()
        else:
...
class MAMBAAD(nn.Module):
    def __init__(self, model_t, model_s):
        ...
        self.mff_oce = MFF_OCE(Bottleneck, 3, device=device)
